...
public System.Collections.Generic.List<clsVariant> skuVariants; //dicSKUs As Dictionary(Of String, ClsProductVariant) 'distiSKU > our Product/Variant
public int tryCount = 0;

public clsQueuedRequest(int RequestID, clsAccount BuyerAccount, System.Collections.Generic.List<clsVariant> skuVariants) //Dictionary(Of String, ClsProductVariant))
{
...
public static ConcurrentDictionary<int, clsQueuedRequest> PendingRequests; //declared at a module level so available to all users - holds the request handle, and a dictionary of the DistiSKUs>ProductVariants - used when handing the responses in getPriceUIs.aspx to update the correct prices


public static void EmbedUpdateRequest(clsBranchInfo pbi, System.Collections.Generic.Dictionary<clsBranch, clsVisibility> descendantPaths, Panel childrenPanel, ref System.Collections.Generic.List<string> errormessages)
{
...
System.Collections.Generic.List<clsVariant> NeedUpdate = default(System.Collections.Generic.List<clsVariant>); //Dictionary(Of String, ClsProductVariant)
NeedUpdate = new List<clsVariant>(); //Dictionary(Of String, ClsProductVariant)

//needupdate will cointain only out of date, visible products - often none !
foreach (var vs in descendantPaths.Values) //note - the keys are Branches (the values are paths)
{
if (vs.branch.HasSKU) //only fetch prices for things with SKU (ie. NOT for placeholder branches !)
{
...
childrenPanel.Controls.Add(Utility.fetcherImage("tree.1", handle, NeedUpdate));
}
}
...
/// <remarks></remarks>
public static string GetVariants(string hostID, string warehouse, System.Collections.Generic.List<string> errorMessages)
{
...
///
public static int DispatchUpdateRequest(UInt64 lid, System.Collections.Generic.List<clsVariant> needingUpdate, string authkey, System.Collections.Generic.List<string> errorMessages)
{
...
int quoteid = System.Convert.ToInt32(CoreCode.iq.get_sesh(lid, "QuoteID"));
...
int handle = 0; //each request gets a handle (also appears in the header)
...
//Prepare a request
object[] skus = (from i in needingUpdate select i.DistiSku).ToArray; //LINQ
if (skus.Count == 0)
{
return 0;
}
...
//REMOVE
if (buyerAccount.SellerChannel.IsCloneOf.Code == "DWERG74AH" && string.IsNullOrEmpty(buyerAccount.Priceband.text))
{
buyerAccount.Priceband = CoreCode.iq.getPriceBand("CHA097");
}
...
wsconsumer.clsStockPriceRequest request = default(wsconsumer.clsStockPriceRequest);
string qid = "";
clsQuote quote = default(clsQuote);
if (quoteid != 0)
{
if (CoreCode.iq.Quotes.ContainsKey(quoteid))
{
quote = CoreCode.iq.Quotes(quoteid);
qid = quote.RootItem.ID + "-" + System.Convert.ToString(quote.Version) + "(" + System.Convert.ToString(quote.ID) + ")";
}
}

string WSRQKey = buyerAccount.Priceband.text;
if (CoreCode.iq.SeshContains(lid, "gk_SessionID"))
{
WSRQKey += ";" + System.Convert.ToString(CoreCode.iq.get_sesh(lid, "gk_sessionID"));
}
...
wsCall = "BuildRequest";
request = Requester.BuildRequest(buyerAccount.SellerChannel.IsCloneOf.Code, buyerAccount.Priceband.text, (agentAccount.User.ID).ToString(), (lid).ToString(), agentAccount.Currency.Code, "", WSRQKey, skus, qid, agentAccount.User.Email, "iQuote2");
wsCall = null;
...
if (PendingRequests.TryAdd(handle, new clsQueuedRequest(handle, buyerAccount, needingUpdate))) //store the request info - including the dictionary we will use to look up the product variants from the HostsSKUs
{
string ms = Utility.TimeSince(ref ts);
if (Conversion.Val(ms) > 100)
{
...
{
ErrorLog.Add(new Exception("DispatchUpdateRequest - Pending requests already contained the handle " + System.Convert.ToString(handle) + " (IQ2 Restarted ?)"));
return -1;
}
...
return 0;
//Finally
//Profiling.Pacc("WS_DispatchUpdateRequest");
}
