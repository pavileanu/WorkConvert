...
public class IQDrive
{
	...
	public static string uploadFile(string filepath, string certpath)
	{
		...
		try {
			...
			object scope = new List<string>();
			scope.Add(DriveService.Scope.Drive);
			object certificate = new X509Certificate2(certpath, "notasecret", X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.Exportable);
			...
			object service = new DriveService(serviceInitialiser);
			...
			if (file != null) {
				...
				pdffilepath = IO.Path.GetFullPath(filepath) + IO.Path.GetFileNameWithoutExtension(filepath) + IO.Path.GetExtension(filepath) == ".ods" ? ".pdf" : IO.Path.GetExtension(filepath);
