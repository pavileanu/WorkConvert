public class clsManipPrune : ManipulationMethod
{
    ...
    {
        wipeCachedDataView(SourcePath, LoginId);
        System.Collections.Generic.KeyValuePair<int, clsPrune> pid = iq.Branches(SourceBranch.ID).Prunes.Where(p => p.Value.Path == SourcePath && p.Value.ChannelID.value == DBNull.Value).FirstOrDefault();
        if (pid.Value != null)
        {
