public class clsAccountScreenField
{
    ...
    public bool HasScreenOverride
    {
        get
        {
            return iq.ScreenOverrides.ToList().Where(a => a.AccountID == AccountID && a.FieldId == FieldId && a.ScreenID == ScreenID && a.Path == Path).Count() > 0;
        }
    }
    ...
    public string DisplayUnitSymbol
    {
        get
        {
            return System.Convert.ToString(DisplayUnit == null ? string.Empty : DisplayUnit.Symbol);
        }
    }



    public dynamic ConvertValueToUnit(long value, int? origialUnit)
    {
        if (DisplayUnit == null)
        {
            return value;
