...
public class clsBranchInfo
{
		...
		//Log In Id (session key)
	public UInt64 lid;
		//read-only          'path in the tree of this branch eg. 'tree.1.4.563.8383.3993
	public  path;
		...
		if (screenHeaders.ContainsKey(this.path)) {
			...
			if (getBranchStateAbove(lid, path, errorMessages) != null) {
				if (getBranchStateAbove(lid, path, errorMessages).rca == enumBt.DetailSquare | getBranchStateAbove(lid, path, errorMessages).rca == enumBt.Branch) {
					descendants = this.visibleChildren(errorMessages, United, 0, 0, true, true);
					ScreenHeader = new clsScreenHeader(this, descendants, United);
				} else if (descendants != null) {
					ScreenHeader = new clsScreenHeader(this, descendants, United);
				// getBranchStateAbove(lid, path, errorMessages).rca = enumBt.gridrow Then
				} else if (rootPath != null) {
					_EffectiveHeader = matrixHeaderAbove(this.lid, rootPath == null ? path : rootPath, errorMessages);
					//provides a reference to the clsScreenHeader in force for this branch - for this user-session (for formatting)                  
				}
			} else {
				_EffectiveHeader = matrixHeaderAbove(this.lid, rootPath == null ? path : rootPath, errorMessages);
				//provides a reference to the clsScreenHeader in force for this branch - for this user-session (for formatting)
	...
	public clsScreen EffectiveMatrix {
		get { return MatrixAbove(this.lid, this.path); }
	}
	...
	public bool treeMode;
		//Renders everything as a branch, and with extra buttons (prune, graft etc.)
	public bool branchButtons;
	public float treeWidth;
		//How to refer to this branches lone child - eg 'Option','System','family'
	public string collectiveSingle;
		//How to refer to this branches children (plural) - eg 'Options','Systems','families'
	public string collectivePlural;
		//Addional filtering of products by their focus attribute (Receta being the prime example)
	public HashSet<string> foci;
		//number of rows 'suriving' the 'Quick' filtering 
	public int survivors;
	public enumParadigm Paradigm;
	public int rownum;
		//used by processcommand to return the DivToFill (either 'path' or "tree" (for OpenTO's)
	public string divToFill;
		//Used to conver swift and shopping list warnings/exceptions
	...
	public clsBranchInfo(UInt64 lid, string path)
	{
		...
		this.showAll = (bool)iq.sesh(lid, "showAll");
		this.agentAccount = (clsAccount)iq.sesh(lid, "AgentAccount");
		this.buyerAccount = (clsAccount)iq.sesh(lid, "BuyerAccount");
		this.foci = new HashSet<string>(Split((string)iq.sesh(this.lid, "foci"), ",").ToList);
		this.userMessages = new List<string>();
	}


	public clsBranchInfo(UInt64 lid, path, Label lblMatches, float treewidth, enumParadigm Paradigm, ref List<string> errorMessages, string rootPath = null)
	{
		...
		if (IsNumeric(Split(path, ".").Last)) {
			this.branch = iq.Branches((int)Split(path, ".").Last);
		}

		this.showAll = (bool)iq.sesh(lid, "showAll");
		this.treeMode = (bool)iq.sesh(lid, "treeMode");

		//If Me.ShowAll Then Stop
		this.branchButtons = (bool)iq.sesh(lid, "branchButtons");
		//
		...
		if (this.ScreenHeader != null && this.ScreenHeader.Vw != null) {
			this.survivors = this.ScreenHeader.Vw.Count;
		...
		if (iq.SeshContains(lid, "treeCursorPath")) {
			if (LCase(path) == (string)iq.sesh(lid, "treeCursorPath"))
		...
		if (!this.agentAccount == null) {
			if (p.Count > 0) {
				clsBranch parentbranch = iq.Branches((int)p.Last);
				this.collectiveSingle = parentbranch.collectiveNounSingular.text(this.agentAccount.Language);
				//get the name of the things we're viewing from the first one of them (e.g. servers)
				this.collectivePlural = parentbranch.CollectiveNoun.text(this.agentAccount.Language);
			} else {
				this.collectiveSingle = iq.RootBranch.collectiveNounSingular.text(this.agentAccount.Language);
				//get the name of the things we're viewing from the first one of them (e.g. servers)
				this.collectivePlural = iq.RootBranch.CollectiveNoun.text(this.agentAccount.Language);
			}
		}
	...
	/// <returns></returns>
	/// <remarks></remarks>
	public Dictionary<clsBranch, clsVisibility> visibleChildren(ref List<string> errorMessages, bool united, ref int skus, ref int cats, bool fbv, bool filter, bool checkSystem = false)
	{

		Dictionary<clsBranch, clsVisibility> vc;
		...
		clsScreenHeader eh = null;
		//When recalcualting filters - we need to NOT apply the view at this level - but DO apply upper filters in effect
		string ehp = this.path;
		//effective header path
		if (fbv == false)
			ehp = Left(ehp, InStrRev(ehp, ".") - 1);
		//filter by view
		if (ehp == "tree")
			return vc;

		eh = EffectiveHeader;
		//matrixHeaderAbove(Me.lid, ehp, errorMessages)
		...
		if (fbv && eh != null && eh.Vw != null && eh.Vw.Count > 0) {
			...
			Dictionary<clsBranch, clsVisibility> filteredAndSorted = new Dictionary<clsBranch, clsVisibility>();
				...
				//vc are the visible children (clsVisibility)  - they may be SKU'd product branches or SKUless category branches
...
enumBt.DetailSquare:
					if (!branchstate.United == false)
						InvalidateMatrixBelow(this.path);

					Dictionary<clsBranch, clsVisibility> descendants = this.visibleChildren(errorMessages, true, 0, 0, true, true);
					branchstate.rca = enumBt.DetailSquare;
					// Ok - now Render your Children As detailed squares 
...
enumBt.gridrow:
					...
					CloseBelow(this.lid, this.path);
					//this is really important - we need to kill all state below the branch we're swithing to a grid (otherwise (for example) supply chains will cause gridrows to render as branches - see GetBranchstateAbove)
					...
					branchstate.rca = enumBt.gridrow;
					// Ok - now Render your Children As gridrows 
	...
	private void InvalidateMatrixBelow(string path, bool IncludeActualPath = false)
	{
	...
	public void close(ref List<string> errorMessages)
	{
	...
	public clsBranchState open(ref List<string> errorMessages, bool openFirstTab)
	{
		//BranchState is a RERERENCE to an object utimately stored under sesh(lid)
		...
		if (branchstate == null || branchstate.rca == enumBt.Hidden) {
			...
			bool unite = (bool)branch.rca.First == "G";
			enumBt bt;
			bt = (enumBt)BTchar.IndexOf(this.branch.rca.First);
			//use the default form from the branches RCA
			if (branchstate == null) {
		...
		if (branchstate.rca == enumBt.gridrow | branchstate.rca == enumBt.Tab) {
			...
			if (treeMode)
				fbv = false;
			Dictionary<clsBranch, clsVisibility> descendants = this.visibleChildren(errorMessages, branchstate.United, 0, 0, fbv, true);
			...
			if (branchstate.rca == enumBt.gridrow) {
				...
				//NOT at the level of an option (with no descendants) though - or bad things happen
				if (descendants.Count > 0 & this.ScreenHeader == null) {
					Dictionary<string, clsScreenHeader> matrixHeaders = (Dictionary<string, clsScreenHeader>)iq.sesh(lid, "screenHeaders");
					if (!matrixHeaders.ContainsKey(this.path)) {
				...
				//From j In descendants.Values Order By iq.Branches(CInt(Split(j.path, ".").Last)).order
				foreach ( vi in descendants.Values) {
				...
				if (!tabOpen) {
					//clicking the hyperlink twice do descendants here
					if (firstTab != null) {
						firstTab.open(errorMessages, openFirstTab);
						//if none of the tabs in this set are open - we open the first (actually this just sets state - it will be rendered open)
					}
				}
			}
		}
		...
		if (branchstate != null) {
			//AUTOOPEN - 'O' type branches
			if (branchstate.rca == enumBt.OpenBranch) {
				foreach ( CB in this.branch.childBranches.Values) {
	...
	{
		return path.Split((char)".").Length - 1;
	}
